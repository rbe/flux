...
GLUE.UI.WIDGET = {
    ...
    initLiveSearch: function(map) {
        // Live Search
        jQuery('#search-bar input[name="q"]').liveSearch({url: map.url + '?q='});
    },
    ...
    renderContentBoxContent: function(map) {
        ...
        jQuery(contentSel).append(GLUE.UI.evalContent(map.content));
    },
    ...
    renderSection: function(map) {
        ...
        text += '<h2>' + GLUE.UI.evalContent(map.title) + '</h2>';
        text += '<span>' + GLUE.UI.evalContent(map.subtitle) + '</span>';

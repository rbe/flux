 ...
 * 
 */
var FLUX=function(d){var a={URL_PREFIX:FLUX.API_URL_PREFIX+"/flux",FLUX_URL:FLUX.API_URL_PREFIX+"/flux/"+FLUX.VERSION.FLUX+"/js",JQUERY_URL:FLUX.API_URL_PREFIX+"/jquery/"+FLUX.VERSION.JQUERY+"/jquery.min.js",ADMINTASIA_URL:FLUX.API_URL_PREFIX+"/admintasia/"+FLUX.VERSION.ADMINTASIA+"/js",},c={PASTE:224,},b={API_URL_PREFIX:"http://api.art-of-coding.eu",VERSION:{FLUX:"2.0.0",JQUERY:"1.7.1",ADMINTASIA:"2.1",},};b.makeHttpRequest=function(){if(window.XMLHttpRequest){return new XMLHttpRequest()}else{if(window.ActiveXObject){return new ActiveXObject("MsXml2.XmlHttp")}}};b.ajaxPage=function(f){var e=this.getHttpRequest();e.onReadyStateChange=function(){if(e.readyState==4){if(e.status==200||e.status==304){this.includeJavaScript(e.responseText)}else{alert("XML request error with "+f+": "+e.statusText+" ("+e.status+")")}}};e.open("GET",f,true);e.send(null)};b.includeJavaScript=function(g){if(g!=null){var e=document.createElement("script");e.language="JavaScript";e.type="text/javascript";e.text=g;var f=document.getElementsByTagName("head").item(0);f.appendChild(e)}};b.loadScript=function(f,h){var e=document.createElement("script");e.language="JavaScript";e.type="text/javascript";e.src=f;if(h){e.onReadyStateChange=function(){if(this.readyState=="loaded"||this.readyState=="complete"){h()}};e.onload=function(){if(h){h()}}}var g=document.getElementsByTagName("head")[0];g.appendChild(e)},b.loadScripts=function(f,e,g){if(!e){e=0}if(e<f.length){FLUX.loadScript(f[e],function(){FLUX.loadScripts(f,++e,g)})}else{if(g){g()}}};b.init=function(f){var e=function(){if(f){f()}jQuery("#page_wrapper").show()};this.loadScript(this.META.JQUERY_URL);this.loadScript(this.META.FLUX_URL+"/flux.log.js");this.loadScript(this.META.FLUX_URL+"/flux.tools.js");this.loadScript(this.META.FLUX_URL+"/flux.fastms.js");this.loadScript(this.META.FLUX_URL+"/flux.ui.js",function(){FLUX.UI.init(e)})};return b};if(typeof jQuery!=="undefined"){FLUX=FLUX(jQuery)}else{alert("Could not initialize FLUX: jQuery not loaded.")}
/*!
 * Flux
 * jQuery Extras.
 * 
 ...
 * 
 */
var FLUX=FLUX||{};FLUX.JQUERY=function(b){var a={};a.init=function(){jQuery.fn.extend({insertAtCaret:function(c){return this.each(function(f){if(document.selection){this.focus();sel=document.selection.createRange();sel.text=c;this.focus()}else{if(this.selectionStart||this.selectionStart=="0"){var e=this.selectionStart;var d=this.selectionEnd;var g=this.scrollTop;this.value=this.value.substring(0,e)+c+this.value.substring(d,this.value.length);this.focus();this.selectionStart=e+c.length;this.selectionEnd=e+c.length;this.scrollTop=g}else{this.value+=c;this.focus()}}})}})};return a};
/*!
 * Flux
 * Logging.
 * 
 ...
 * 
 */
FLUX.LOG=function(b){var a={};this._wrConsole=function(c){console.log(new Date().toLocaleString()+" "+c)};a.debug=function(c){this._wrConsole("  DEBUG: "+c)};a.error=function(c){this._wrConsole("  ERROR: "+c)};a.warning=function(c){this._wrConsole("WARNING: "+c)};a.info=function(c){this._wrConsole("   INFO: "+c)};return a};
/*!
 * Flux
 * JavaScript tools.
 * 
 ...
 * 
 */
FLUX.TOOLS=function(b){var a={};a.merge=function(d,c){for(z in c){d[z]=c[z]}return d};return a};